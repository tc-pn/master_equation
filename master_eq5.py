...
PATH = '/home/thomas/Bureau/project_master5'#'/projet/pth/czuba/2021/tdhfbproj/master_eq_gamma_2/subspace' # path of the project, can be changed directly

#################################################
# Solver (= main function)
#################################################

def markov_solver(A,N,Npas,dt,nu,gamma_conserv):
# the markov equation solver

    epsilon = sp_ener(S,A)

    basis, basis_ten, shape_basis = gen_many_body_basis(A,N) # generation of the basis, shape of the basis is ((A choose N),A)
    ...
    for i in range(size_basis):
        mb_energies[i] = np.sum(epsilon * basis[i,:]) # energy of each state of the basis

    gs_energy = mb_energies[0]
    ...
    BASIS_DIC = [] # this array contains: the 10 representation of the state, its energy, its population, and the indexes of the states corresponding to its 2p2h transitions
    mb_weights = list(mb_weights)
    nbr_accepted_states = 0
    for i in range(size_basis):
        diff_ini = np.abs(mb_energies[i] - ini_energy)
        if diff_ini <= gamma_conserv: # selection of the Hilbert subspace that conserve energy up to gamma_conserv
            BASIS_DIC.append([basis_ten[i],mb_energies[i],mb_weights[i]]) # adding 10 representation and energy
            nbr_accepted_states = nbr_accepted_states + 1


    print('selection rules')
    for i in range(nbr_accepted_states):
        ten_num = BASIS_DIC[i][0]
        bin_num = tobin(ten_num,A)
        MpMh_indexes = []
        for j in range(nbr_accepted_states):
            ten_num2 = BASIS_DIC[j][0]
            bin_num2 = tobin(ten_num2,A)
            mask = mask_gen(bin_num2,bin_num,M) # construction of the mask : is the transition 2p2h ?
            if mask == True:
                MpMh_indexes.append(j)
        BASIS_DIC[i].append(MpMh_indexes)

    test = time.time() - test
    print('dic finished', test, 's')

# initialization
    newweights = []
    for i in range(len(BASIS_DIC)):
        newweights.append(BASIS_DIC[i][2])
    newweights = np.array(newweights)
    oldw = np.copy(newweights)
    save_weights = np.copy(newweights)

#    save_energy = newweights * mb_energies

    test = time.time()
    Gain, Loss = GL_terms(BASIS_DIC,PATH,A,N,M,ini_energy,gs_energy)
    test = time.time() - test
...
# RK2 propagation : optimzed by using matrix multiplication
    ...
    print(np.shape(Gain),np.shape(Loss),np.shape(oldw))

    for it in range(Npas):

# t + dt/2
#        RHSa = np.matmul(Gain, np.transpose(oldw)) #- np.sum(Loss * oldw, axis=0)

        for i in range(len(BASIS_DIC)):
            RHSa = 0.
            for j in range(len(BASIS_DIC[0][3])):
                RHSa = RHSa + Gain[i,j] * oldw[i]
            RHSa = RHSa - np.sum(Loss * oldw, axis=0)[i]

            newweights[i] = oldw[i] + dt / 2. * RHSa
# t + dt

        for i in range(len(BASIS_DIC)):
            RHSb = 0.
            for j in range(len(BASIS_DIC[0][3])):
                RHSb = RHSb + Gain[i,j] * newweights[i]
            RHSb = RHSb - np.sum(Loss * newweights, axis=0)[i]

            newweights[i] = newweights[i] + dt * RHSb

#        RHSb = np.matmul(Gain, np.transpose(newweights)) #- np.sum(Loss * newweights, axis=0)
#        RHSb = RHSb - np.sum(Loss * newweights, axis=0)
#
...
def toten(array,A): # convert a binary number into its base 10 representation
    return np.sum(2. ** np.arange(0,A) * array).astype(int)
...
# i use that instead of the energy because there can be degenerated states, and the sort will not work properly
    ...
    for i in range(size_basis):
        ten_repr[i] = toten(basis[i,:],A)  # conversion binary numbers into base 10 number: unique label for all states
    ...
    print('basis ordered')
    return basis, ten_repr, shape_basis

#################################################
# construction of the mask for selection rules : is the transition MpMh ?
#################################################

def mask_gen(bin_num2,bin_num,M):

    test = bin_num2 - np.array(bin_num)
